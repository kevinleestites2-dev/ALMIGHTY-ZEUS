...
class SAFLAEngine:
    ...
    async def ignite(self, mission):
        print(f"[*] SAFLA ENGINE IGNITED. MISSION: {mission}")
        while self.active:
            # Full loop cycle using fused logic
            result = await self.bot.process(mission)
            if "OBJECTIVE_SECURED" in result: break
            await asyncio.sleep(1)

if __name__ == "__main__":
    master = ZeusPrime()
    safla = SAFLAEngine(master)
    asyncio.run(master.start())
    asyncio.run(safla.ignite("Sovereign Evolution"))

# --- GPT-PRIME NEURAL UPLINK INTEGRATION ---
...
# Fable 5 replaces OpenAI across the Pantheon
OPENROUTER_API_KEY  = os.getenv("OPENROUTER_API_KEY", "")
OPENROUTER_MODEL    = os.getenv("OPENROUTER_MODEL", "anthropic/claude-fable-5")
OPENAI_API_KEY      = os.getenv("OPENAI_API_KEY", "")  # legacy fallback only
